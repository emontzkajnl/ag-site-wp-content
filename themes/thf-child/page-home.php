			...
			<?php } else if(get_option('mvp_feat_post') == 'Featured Posts 2') { ?>
				...
				<?php $mvp_slider = get_option('mvp_slider'); if ($mvp_slider == "true") { ?>
					...
					<div id="feat1-left-wrapper">
						...
						<?php if (!empty($do_not_duplicate)) { $recent = new WP_Query(array( 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
				...
				<?php } else { ?>
					...
					<div id="feat1-main-wrapper">
						...
						<?php $recent = new WP_Query(array('tag' => get_option('mvp_slider_tags'), 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
					...
					<div id="feat1-left-wrapper">
						...
						<?php if (!empty($do_not_duplicate)) { $recent = new WP_Query(array('tag' => get_option('mvp_slider_tags'), 'post__not_in' => $do_not_duplicate, 'posts_per_page' => '1'  )); while($recent->have_posts()) : $recent->the_post(); $do_not_duplicate[] = $post->ID; if (isset($do_not_duplicate)) { ?>
...
<!-- Featured Posts 3 -->
	...
	<?php } ?>
<!-- EVENTS -->
<div class="home-events-filter-wrapper">
	<div class="home-events-filter">
		<h3 class="home-widget-header">Find Tennessee Events Near You</h3>
		<?php echo do_shortcode('[searchandfilter id="17721"]'); ?>
	</div>
</div>
<!-- Main Content -->
						...
						<?php $mvp_slider = get_option('mvp_slider'); $mvp_posts = get_option('mvp_featured_posts'); if (($mvp_slider == "true") || ($mvp_posts == "true")) { ?>
							...
							<li class="infinite-post">
								...
								<div class="wide-img">
									...
									<?php endif; ?>
									<a href="/<?php $category = get_the_category(); echo $category[0]->slug; ?>" rel="bookmark">
				...
				<div class="home-widget">
					...
					<ul class="split-columns infinite-content">
						...
						<?php $mvp_slider = get_option('mvp_slider'); $mvp_posts = get_option('mvp_featured_posts'); if (($mvp_slider == "true") || ($mvp_posts == "true")) { ?>
							...
							<?php if (isset($do_not_duplicate)) { $mvp_posts_num = get_option('mvp_posts_num'); $paged = (get_query_var('page')) ? get_query_var('page') : 1; query_posts(array( 'posts_per_page' => 6, 'post__not_in'=>$do_not_duplicate, 'category__not_in'=> array( 2522, 1 ), 'paged' =>$paged )); if (have_posts()) : while (have_posts()) : the_post(); ?>
							...
							<li class="infinite-post">
								...
								<div class="split-img">
									...
									<noscript><a href="<?php the_permalink(); ?>" rel="bookmark"><?php the_post_thumbnail('medium-thumb'); ?></a></noscript>
...
<!-- 
 ...
 -->
				...
				<span class="second-title"><?php if (!function_exists('dynamic_sidebar') || !dynamic_sidebar('Homepage Widget Area')): endif; ?></span>
				</div>
				<?php } ?>
