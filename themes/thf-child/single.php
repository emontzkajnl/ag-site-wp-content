			...
			<div id="home-main">
				...
				<div id="post-area" itemscope itemtype="http://schema.org/Article" <?php post_class(); ?>>
					...
					<div id="post-info">
						...
						<?php _e( 'By', 'mvp-text' ); ?>&nbsp;<span class="author" itemprop="author"><?php the_author_posts_link(); ?></span>
						<br /><time class="post-date" style="line-height:1.3em;" itemprop="datePublished" datetime="<?php the_time('Y-m-d'); ?>" pubdate><?php the_time(get_option('date_format')); ?></time>
						<?php $originaldate = get_field('original_date'); ?>
						<?php if ($originaldate) { ?>
						<?php if (!is_mobile()) { ?> | <?php } else { ?> <br /> <?php } ?><span style="font-style:italic; line-height:1.3em;">Originally Published: <?php the_field('original_date') ?></span>
						<?php } ?>
					</div><!--post-info-->
					
					<?php if (has_tag('youth-in-ag')) { ?>
					<div class="sponsored-by" style="width:100%; background:#eee; padding:8px 0; margin:1em 0 2em; font-style:italic; text-align:center; display: block; float:left;">
						Sponsored by: <a href="https://apply.fcma.com/?utm_source=Article&utm_medium=YIA+Article&utm_campaign=Brand+Apply+Now&utm_id=Home+and+Farm+Article&utm_content=Brand+Apply+Now">Farm Credit Mid-America</a>
					</div>
					<?php } ?>
